...
def extract_key_points(text, output_lang="en"):
    try:
        prompt_text = text[:15000] # Increased limit for longer texts
...
If the text explicitly mentions a specific number of rules, steps, or laws (e.g., "5 rules of SEO", "3 methods"), you MUST find and extract those exact points as the main focus of your infographic.
Output EXACTLY 6 points. (If the text has 5 rules, the 1st point can be an Intro, and the next 5 are the rules).
...
2. Use FontAwesome 6 free solid icons (e.g. fa-rocket, fa-brain, fa-chart-line, fa-bullseye, fa-star, fa-cogs, fa-users, fa-check).
...
{prompt_text}"""
        ...
        for attempt in range(3):
            try:
                ...
                print(f"G4F API Error on attempt {attempt+1}: {e}")
                import time
                time.sleep(2)
                continue
    except Exception as e:
        print(f"G4F AI failed entirely: {e}")
        pass

    # Clean text to ensure proper sentence boundaries
    text = text.replace('؛', '.').replace('\n', '. ').replace('?', '.').replace('!', '.')
    
    # Forceful sentence splitting for transcripts that lack punctuation completely
    if text.count('.') < 5:
        words = text.split()
        text = ""
    ...
    # Split sentences and filter empty/very short ones
    ...
    # 1. Introduction (First meaningful sentence)
    if sentences:
    ...
    # 2. Key Concepts (Sentences containing the top keywords)
    for kw in keywords:
        if len(selected_items) >= 5: # Leave room for conclusion
            break
    ...
    # 3. Conclusion (Last meaningful sentence)
    ...
    # Fill with remaining sentences if we don't have enough
...
{prompt_text}"""
        ...
        for attempt in range(3):
            try:
                ...
                print(f"G4F E-Book API Error on attempt {attempt+1}: {e}")
                import time
                time.sleep(2)
                continue
    except Exception as e:
        print(f"G4F AI E-Book failed entirely: {e}")
        pass

    # Fallback to offline NLP E-book generation
...
def video_to_inograf(request):
    if request.method == 'POST' and request.FILES.get('video_file'):
        ...
        temp_video_path = os.path.join(settings.BASE_DIR, f'temp_video_ino_{session_id}{video_ext}')
        temp_audio_path = os.path.join(settings.BASE_DIR, f'temp_audio_ino_{session_id}.wav')
        ...
        try:
            ...
            if action == 'ebook':
                ...
                # 4. Generate Infographic JSON using extracted text
                key_points = extract_key_points(extracted_text.strip(), output_lang=output_language)
